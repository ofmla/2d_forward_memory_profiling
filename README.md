...
| | |
|:-------------------------:|:-------------------------:|
|<img width="1604" alt="screen shot 2017-08-07 at 12 18 15 pm" src="https://github.com/ofmla/2d_forward_memory_profiling/blob/broadcast_solver/figs/Figure_1.png">|  <img width="1604" alt="screen shot 2017-08-07 at 12 18 15 pm" src="https://github.com/ofmla/2d_forward_memory_profiling/blob/broadcast_solver/figs/Figure_2.png">|
|<img width="1604" alt="screen shot 2017-08-07 at 12 18 15 pm" src="https://github.com/ofmla/2d_forward_memory_profiling/blob/broadcast_solver/figs/Figure_3.png">|  <img width="1604" alt="screen shot 2017-08-07 at 12 18 15 pm" src="https://github.com/ofmla/2d_forward_memory_profiling/blob/broadcast_solver/figs/Figure_4.png">|
...
In the parallel cases, I saw a remarkable increase in the memory in a first experiment ([main](https://github.com/ofmla/2d_forward_memory_profiling/tree/main) branch), which I assumed that is related to the issue [#845](https://github.com/devitocodes/devito/issues/845). Therefore, I decided to reuse Devito objects as much possible, that is, one `solver` or `Operator` object is created ans sent to the dask processes and `(Sparse)TimeFunctions` are reused for modeling several shots by the dask processes. in order to be able to reuse the `(Sparse)TimeFunctions`, it was necessary to modify the way of distributing the shots. Thus, original list of shots was break up into many lists. In other words a list of lists was divided up among the dask processes. In doing so, I maintained the memory consumption nearly flat. **It is important to stress that this strategy should be used because the memory is not released on successfull completion of the forward modeling for a shot.**
